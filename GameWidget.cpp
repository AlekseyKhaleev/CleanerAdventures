...
: QWidget{parent}, m_mazeModel(new Maze::MazeModel), m_gameOverView(new GameOverView)
{
    ...
    m_scoreView   = new ScoreView{4, m_robotModel->getRobotModel()};
    ...
    connect(m_gameOverView, SIGNAL(gameStarted(int)), m_controller, SLOT(startGame(int)));
    ...
    //********* m_controller sender ************
    connect(m_controller, SIGNAL(gameLosed()), m_gameOverView, SLOT(levelDefeated()));
    connect(m_controller, SIGNAL(gameWon()), m_gameOverView, SLOT(levelDone()));
    connect(m_controller, SIGNAL(resetMaze(int)), m_mazeModel, SLOT(initMaze(int)));
    connect(m_controller, SIGNAL(resetRobot(Maze::Model)), m_robotModel, SLOT(initRobot(Maze::Model)));
    ...
    connect(m_controller, SIGNAL(EnergyChanged(int)),              m_robotModel, SLOT(setRobotEnergy(int)));
